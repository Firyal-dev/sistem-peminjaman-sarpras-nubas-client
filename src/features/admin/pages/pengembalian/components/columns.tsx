...
export const columns: ColumnDef<ApiTransaction>[] = [
  ...
  },
  {
    ...
    header: "No",
    cell: ({ row }) => (
      <div className="w-[40px] font-mono text-muted-foreground">
        {row.getValue("id")}
      </div>
    ),
  },
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex flex-col gap-1">
          <Badge
            variant="secondary"
            className="w-fit gap-1 bg-green-100 text-green-700 border-green-200 text-xs"
          >
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const displayText = hasNotes ? notes : "-";
      const isLongText = hasNotes && notes.length > 50;
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="text-sm text-muted-foreground cursor-help line-clamp-2 max-w-xs break-words">
                  {displayText}
                </div>
              </TooltipTrigger>
