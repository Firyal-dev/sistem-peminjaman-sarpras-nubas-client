...
export const studentsService = {
    getAll: (classId?: number) =>
        api.get<ApiStudent[]>('/students', { params: classId ? { class_id: classId } : {} })
            .then(r => r.data),
    create: (data: { name: string; nis: string; class_id: number }) =>
        api.post<ApiStudent>('/students', data).then(r => r.data),
    update: (id: number, data: Partial<{ name: string; nis: string; class_id: number }>) =>
...
export const itemsService = {
    ...
    create: (data: { name: string; photo?: string | null }) =>
        api.post<ApiItem>('/items', data).then(r => r.data),
    update: (id: number, data: Partial<{ name: string; photo?: string | null }>) =>
        api.put<ApiItem>(`/items/${id}`, data).then(r => r.data),
...
export const unitsService = {
    ...
    generate: (itemId: number, jumlah: number) =>
        api.post<ApiUnit[]>(`/items/${itemId}/units`, { jumlah }).then(r => r.data),
...
export const scanService = {
    borrowScan: (qrCode: string) =>
        api.post<ScanBorrowResponse>('/scan', { qr_code: qrCode }).then(r => r.data),
    returnScan: (qrCode: string, transactionId: number) =>
        api.post('/return/scan', { qr_code: qrCode, transaction_id: transactionId }).then(r => r.data),
}

// ─── Transactions ─────────────────────────────────────────────────────────────

export const transactionsService = {
    getAll: (params?: { student_id?: number; status?: 'active' | 'done'; page?: number }) =>
        api.get<PaginatedResponse<ApiTransaction>>('/transactions', { params }).then(r => r.data),
    getOne: (id: number) =>
    ...
    processReturn: (transactionId: number, unitIds: number[]) =>
        api.post<ApiTransaction>(`/transactions/${transactionId}/return`, { units: unitIds })
            .then(r => r.data),
